...
let classifier: any;
let patterns: string[] = ['no pattern specified', 'solid', 'bicolor', 'tabby', 'tortoiseshell', 'calico', 'colorpoint', 'mink', 'sepia', 'smoke', 'shaded', 'chinchilla'];
let colors: string[] = ['no color specified', 'white', 'black', 'blue (grey)', 'brown (chocolate)', 'silver', 'golden', 'cream', 'fawn', 'isabella', 'seal', 'sable (dark brown)', 'champagne (warm beige)', 'platinum (pale grey-lavender)', 'lilac', 'cinnamon', 'orange (red)'];
...
async function classifyImage(imageElement : HTMLImageElement) {
    try {
        ...
        console.error(error);
        (document.getElementById('result' as string) as HTMLElement).innerText = "Classification failed.";
    }
}

(document.getElementById('upload' as string) as HTMLElement).addEventListener('change', function(event: Event) {
    const target = event.target as HTMLInputElement;
    const file: File | undefined = target.files?.[0];
    if (file) {
        const reader = new FileReader();
        reader.onload = function(e: ProgressEvent<FileReader>) {
            const img: HTMLImageElement = document.getElementById('inputImage') as HTMLImageElement;
            img.src = (e.target as FileReader).result as string;
            img.style.display = 'block';
            
            // Wait for the image to load before classifying
            img.onload = async function() {
                (document.getElementById('result' as string) as HTMLElement).innerText = "Classifying...";
                await classifyImage(img as HTMLImageElement);
            }
        }
...
const pets_container: HTMLElement = document.getElementById("pets-container") as HTMLElement;

const patternSelect: HTMLSelectElement = document.getElementById("pattern") as HTMLSelectElement;
const colorSelect: HTMLSelectElement = document.getElementById("color") as HTMLSelectElement;
const breedSelect: HTMLSelectElement = document.getElementById("breed") as HTMLSelectElement;
...
function createTextElement(text: string, id: number, font_size: string, font_weight: string) {
    const p = document.createElement("p");
    p.textContent = text;
    p.style.fontSize = font_size;
    p.style.fontWeight  = font_weight;
    p.style.lineHeight = "0.1";
    (document.getElementById(id.toString()) as HTMLElement).appendChild(p);
}

function displayPets(petsArr = pets) {
    let div_pet: HTMLElement | null, img: HTMLImageElement;
    petsArr.forEach(pet => {
        createPet(pet.id);
        ...
        (div_pet as HTMLElement).appendChild(img);
...
window.onload = async (e) => {
    try {
        ...
        if (!response.ok) {
            ...
            const select: HTMLSelectElement = document.getElementById('breed') as HTMLSelectElement;
...
function displayOptionsFilt() {
    const select_pattern: HTMLSelectElement = document.getElementById('pattern') as HTMLSelectElement;
    ...
    const select_color: HTMLSelectElement = document.getElementById('color') as HTMLSelectElement;
